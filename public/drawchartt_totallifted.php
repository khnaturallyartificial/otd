...
$query = "select `date` , SUM(`pres_total_lifted`) as `stl`, SUM(`actual_total_lifted`) as `satl`
...
while($row = mysql_fetch_array($result)){
              array_unshift($dataar , round(($row["stl"] / 1000),2));
              array_unshift($act_dataar,round(($row["satl"] / 1000),2));
 ...
 $DataSet->SetYAxisName("Weight (tonne)");

 // Cache definition
$Cache = new pCache();
$Cache->GetFromCache("Grapht",$DataSet->GetData());
...
// $Test->drawPlotGraph($DataSet->GetData(),$DataSet->GetDataDescription(),3,2,255,255,255);
 ...
 $Test->drawTitle(60,22,"Prescribed vs Actual total weight lifted (tonne)-",50,50,50,585);

  $Cache->WriteToCache("Grapht",$DataSet->GetData(),$Test);
